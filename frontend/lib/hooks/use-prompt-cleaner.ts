...
const AGENT_HUB_URL = process.env.NEXT_PUBLIC_AGENT_HUB_URL || "https://agent.summitflow.dev";
...
export function usePromptCleaner(): UsePromptCleanerReturn {
  ...
  const cleanPrompt = useCallback(async (prompt: string, refinement?: string): Promise<string> => {
    setIsLoading(true);
    setError(null);
